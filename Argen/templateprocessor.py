...
class LineBuilder:
    ...
    def append(self, text, sep=" ", newline_sep="", first_sep=""):
        if self.cur_line_width == 0:
            ...
            self.curLine.append(newline_sep)
            self.newline()
            self._add_text(text)
    ...
    def join(self, strings, sep=" ", newline_sep="",
             first_sep="", first_newline_sep=""):
        try:
            i = iter(strings)
...
def join(strings, line_width=80, sep=" ", newline_sep="",
         first_sep="", first_newline_sep=""):
...
def _translate_to_space(s, chars):
    # Slow (but not noticeably so in this context) replacement for maketrans and
...
[[[ENDIF]]]
    ...
    return cmdLine;
        ...
        print("\n".join(make_lines(test_string, DictExpander(dict(
            hasInfoOpts=True,
            readInfoOpts=["kjell", "&& arne", "&& bjarne"],
            readHelpOpts="finn \n        || tom",
            incorrectArgCount="count != 3",
            hasNormalOpts=True,
            readNormalOpts="readOptions(\"foo\");",
            hasArgs=True,
            readArgs="readArgs(\"bar\");")))))
        return 0
